...
import DoubleIterator.DoubleIterator;
...
public class DoubleIteratorTest {
  ...
  void worksWithCornerCases() {
    DoubleIterator<Integer> doubleIterator = new DoubleIterator<Integer>(
    ...
    doubleIterator = new DoubleIterator<>(Collections.emptyIterator(),
        List.of(1, 2).listIterator());
    for (int i = 1; i <= 2; ++i) {
      Assertions.assertEquals(i, doubleIterator.next());
    }
    Assertions.assertFalse(doubleIterator.hasNext());

    doubleIterator = new DoubleIterator<>(List.of(1, 2).listIterator(),
        Collections.emptyIterator());
    for (int i = 1; i <= 2; ++i) {
      Assertions.assertEquals(i, doubleIterator.next());
    }
